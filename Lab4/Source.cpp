...
{
    return ((blow_fish.S[0][(x >> 24) & 0xFF] + blow_fish.S[1][(x >> 16) & 0xFF]) ^
        blow_fish.S[2][(x >> 8) & 0xFF]) + blow_fish.S[3][(x) & 0xFF];
}


ValuesFestel encrypt(unsigned long left, unsigned long right)
{
    ValuesFestel values_festel;
    values_festel.left = left;
    values_festel.right = right;

    for (int i = 0; i < 16; i++) {
        values_festel.left = blow_fish.P[i] ^ values_festel.left;
        values_festel.right = values_festel.right ^ F(values_festel.left);
...
ValuesFestel decrypt(unsigned long left, unsigned long right)
{
    ValuesFestel values_festel;
    values_festel.left = left;
    values_festel.right = right;
    ...
    std::swap(values_festel.left, values_festel.right);
    values_festel.left ^= blow_fish.P[0];
    values_festel.right ^= blow_fish.P[1];
...
unsigned long get32(const std::string& str)
{
    ...
    unsigned long v1 = str[0];
    unsigned long v2 = str[1];
...
void extended_open_key(std::string& open_key)
{
    auto buf_key = open_key;
    while (open_key.length() < 72)
        open_key += buf_key;
...
void extended_close_key()
{
    for (int i = 0; i < 18; i++)
        blow_fish.P[i] ^= blow_fish.K[i];

    unsigned long left = 0;
    unsigned long right = 0;
    for (int i = 0; i < 18; i += 2) {
        auto buf = encrypt(left, right);
        left = buf.left;
        right = buf.right;
    }
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 256; j += 2) {
            auto buf = encrypt(blow_fish.S[i][j], blow_fish.S[i][j + 1]);
            blow_fish.S[i][j] = buf.left;
            blow_fish.S[i][j + 1] = buf.right;
        }
    }
